...
An optional baseline date range on the financial query produces exact differences and percentage changes using identical filters and one consistent snapshot. Ranked views group the full saved evidence before sorting and limiting; profit ranking uses net income and refuses incomparable currencies. Zero baselines do not produce invented percentages; currencies remain separate and unequal periods are stated without extrapolation. Superseded list/income tools remain available to external clients, but replacement metadata keeps them out of the new chat's discovery surface.
...
`life.timeline` combines outstanding claims, linked expected cash, recurrence and recorded events, suppressing duplicates and preserving due dates versus expected dates. `life.events` provides indexed text/date/kind search when the question concerns particular occurrences. Relationships, revisions, measurements and source Markdown have separate compact reads. Role and arrangement-name filters return the verified counterpart participants with each relationship, avoiding serial identity lookups. Historical relationship dates use the end of the dataset’s local civil day. Measurement and event pages use date indexes and opaque cursors rather than loading years of records into reference-table arrays. Subject-only event queries use the event-link index, bypassing unrelated financial history; their order is explicitly link order. Measurement cutoffs can cover all recorded history without a guessed start date and return named subjects.
