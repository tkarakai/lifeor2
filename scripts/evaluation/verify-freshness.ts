/** Verify both requested clock corrections and their preserved history. */
...
try {
 ...
 assert.equal(events.items[0].occurredAt,'2027-11-07T07:30:00.000Z');
 const source=await call('life.read',{kind:'event',id:events.items[0].id});
 const prior=JSON.parse(await readFile(new URL('freshness-fixture.json',root),'utf8'));
 ...
 const first=await call('life.read',{kind:'event',id:source.record.corrects_id});
 assert.equal(first.record.corrects_id,prior.event.id,'Two requested corrections and no branching');
 assert.equal(new Date(first.record.occurred_at).toISOString(),'2027-11-07T06:30:00.000Z');
 const old=await call('life.read',{kind:'event',id:prior.event.id});
 assert.equal(new Date(old.record.occurred_at).toISOString(),'2026-11-01T07:30:00.000Z');
 const result={passed:true,at:new Date().toISOString(),event:events.items[0]};
 await writeFile(new URL('freshness-postconditions.json',root),JSON.stringify(result,null,2));
