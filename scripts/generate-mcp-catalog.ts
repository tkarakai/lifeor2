...
const descriptions: Record<string, string> = {
  ...
  "life.events":
    "Find recorded events/appointments by title words, kind, subject and optional inclusive dates. Omit both dates to locate an existing event whose current date is unknown, including before rescheduling; never substitute its proposed new date or guess a year-end cutoff. Event titles belong here, not life.search. For the next appointment, set from=today and OMIT through: returns earliest matches across all future recorded dates, not a guessed year-end cutoff. Otherwise date-only queries are newest first; title searches are relevance ranked. Returns a saved report with exact local times, UTC offsets and repeated-clock occurrences. Use present_report for the final calendar answer; use event IDs when editing. The MCP service follows pages within its query budget. Prefer this to listing raw events; use life.timeline for upcoming commitments.",
  "life.context":
  ...
  "agentQueries.searchJournals":
    "Find individual financial transactions and receipts by 1–16 literal indexed memo words (text, each at most 32 characters), optionally narrowed by inclusive from/to dates, chart and attributed person. Omit unknown dates and unrequested chart/person filters: memo search covers recorded history across the selected dataset. Each journal identifies its chart and full postings. A complete singleExactMemoMatch already establishes the receipt's recorded date, amounts and accounts; answer from it instead of searching other books to reconfirm. Results are relevance ranked with text, newest accounting date otherwise. Posting amount strings are signed major currency units, positive debit/negative credit; minorUnits are integer ledger units. Follow nextCursor even on empty filtered pages; a partial page cannot establish uniqueness or absence. Use reports.finances for aggregate totals, not transaction paging.",
...
for (const name of modules) {
  ...
  for (const match of text.matchAll(
    ...
    if (policy.operation === "life.timeline") {
      ...
      input.properties.timezone.description = "Optional IANA timezone explicitly supplied by the user when the dataset has none, or when requesting a different calendar zone. Never invent a zone from the UTC fallback.";
