...
export function appOrigin() {
  ...
  if (!value || new URL(value).origin !== value)
    throw new Error(
      "Configure NEXT_PUBLIC_SITE_URL as the canonical app origin.",
    );
  ...
  if (
    ...
    throw new Error("Remote agent access requires HTTPS.");
  return value;
}
...
export function checkOrigin(request: Request, require = false) {
  ...
  if (
    ...
    (require && request.headers.get("sec-fetch-site") === "cross-site")
  )
    ...
    return new Response(null, {
      ...
      headers: {
        ...
        "Access-Control-Allow-Headers":
          "Authorization, Content-Type, Accept, Mcp-Method, Mcp-Name, MCP-Protocol-Version",
...
export function safeError(error: unknown): { code: string; message: string } {
  ...
  if (/InternalServerError|Timestamp.*too early|out_of_retention/.test(raw))
    return {
      code: "snapshot_unavailable",
      message:
        "A consistent data snapshot was unavailable. No complete answer was computed. Retry this read once; if it fails again, explain the temporary limitation. Do not change the requested scope or assumptions.",
    };
  const reportFailure = raw.match(/^(QUERY_LIMIT|DATA_CHANGED): (.{1,500})$/);
  if (reportFailure)
    return { code: reportFailure[1].toLowerCase(), message: reportFailure[2] };
  ...
  if (
    domain &&
    !/secret|configuration|environment|fetch|internal/i.test(domain)
  )
    return {
      code: /Revision conflict/.test(domain)
        ? "revision_conflict"
        : "operation_failed",
      message: domain,
    };
  return {
    ...
    message:
      "The operation failed without a usable result. Retry once with unchanged arguments and requestKey. If it fails again, explain the limitation; do not invent an answer or repeatedly change arguments.",
  };
}
