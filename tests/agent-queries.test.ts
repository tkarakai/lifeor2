...
test("receipt resolution distinguishes a complete exact match from partial words, duplicates and later pages", async () => {
  const f = await ledgerFixture();
  const memo = "Pine Street Market receipt";
  await f.t.run(ctx => ctx.db.patch(f.ids[0], { memo, accounting_date: "2012-02-20" }));
  const exact = await f.alice.query(search, { text: "PINE  Street Market receipt" });
  expect(exact).toMatchObject({ complete: true, singleExactMemoMatch: true, filters: { chartId: null, from: null, to: null } });
  expect(exact.records[0]).toMatchObject({ chart: { id: f.chart, name: "Household" }, exactMemoMatch: true, date: "2012-02-20" });
  const words = await f.alice.query(search, { text: "Pine receipt" });
  expect(words.records).toHaveLength(1);
  expect(words.singleExactMemoMatch).toBe(false);
  await f.t.run(ctx => ctx.db.patch(f.ids[1], { memo }));
  const duplicate = await f.alice.query(search, { text: memo });
  expect(duplicate.records).toHaveLength(2);
  expect(duplicate.singleExactMemoMatch).toBe(false);
  const first = await f.alice.query(search, { text: memo, limit: 1 });
  expect(first.singleExactMemoMatch).toBe(false);
  expect(first.nextCursor).toBeTruthy();
  const last = await f.alice.query(search, { text: memo, limit: 1, cursor: first.nextCursor! });
  expect(last.singleExactMemoMatch).toBe(false);
  expect(await f.bob.query(search, { text: memo })).toMatchObject({ records: [], complete: true, singleExactMemoMatch: false });
});
